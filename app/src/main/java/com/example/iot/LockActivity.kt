...
class LockActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        switch2.setThumbTintList(thumbStates)
        ...
        switch2.setTrackTintList(trackStates)
        switch2.setTrackTintMode(PorterDuff.Mode.OVERLAY)
                        ...
                        if(switch2.isChecked) {
                            if(onTime > offTime) {
                                if (time >= onTime || time <= offTime) {
                                    //                          ultratext.text = p0.child("ultra2").value.toString() + " cm"
    ...
    private fun sendNotify(){
        ...
        val bitmap = BitmapFactory.decodeResource(applicationContext.resources,R.drawable.warning)
